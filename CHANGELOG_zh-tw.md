...
- **根集區開機鏡像**（`boot_mirror.py` + `BootMirrorWizard`，管理員）——
  ...
  - **三種情境,用偵測而不是假設。** `add_mirror`(單碟→鏡像)、`replace_live`
    (健康碟的預防性更換)、`replace_dead`(FAULTED / REMOVED / UNAVAIL)。
  ...
  - **伺服器端前置檢查,不過就擋。** 根在 ZFS、目標就是根集區、沒有正在進行的
    清理或重建、有健康的複製來源、新碟是空的、不在集區內、且容量不小於來源
    (整張 GPT 都要複製,所以這項**不能**強制略過)。磁區大小不一致與開機韌體
    列為提醒。
